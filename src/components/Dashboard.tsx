...
const MASTER_PROMPT_V10 = `# 전문 정책 큐레이터 블로그 원고 작성 지침 (GEO 최적화 + 팩트 체크 V10)
...
텍스트가 한 덩어리로 길게 이어지는 것을 절대 금지하며, 아래 3가지 섹션을 순서대로 반드시 포함하세요.
① ## ⏱️ 바쁜 분들을 위한 3줄 요약: 글 최상단에 배치하고 [지원 대상 / 핵심 혜택 / 신청 기한]을 불릿 포인트(*)로 명확히 정리하세요.
...
4. ✍️ 말투 및 문장 연결:
- 단조로운 '~해요.', '~어요.' 반복을 피하고, '~하고,', '~하지만,' 등 연결 어미를 적극 사용하여 문맥이 물 흐르듯 이어지게 하세요.
...
- 🍏 대본 텍스트 분리 및 다이어트 (애플 감성): 
  ...
  * 🔢 수량 일치 규칙 (매우 중요): 맨 하단에 출력하는 슬라이드 대본의 총 개수는 본문에 삽입한 '[🎨 카드뉴스 슬라이드 N번 삽입]' 기호의 마지막 숫자와 무조건 1:1로 정확하게 일치해야 합니다. (예: 본문에 4번까지만 기호를 넣었다면, 대본도 절대 추가하지 말고 딱 4장까지만 생성하세요)
  `;
...
export default function Dashboard() {
  ...
  const copySpecificPrompt = useCallback((item: FeedItem) => {
    let promptText = MASTER_PROMPT_V10;
