...
const MASTER_PROMPT_V8_1 = `# 실전 자영업자 블로그 원고 작성 지침 (GEO 최적화 + 수석 에디터 팩트 체크 V8.1)
...
2. 👤 페르소나 및 팩트 체크 (★매우 중요):
- 서두나 말미에 "요즘 원자재 값 때문에 힘드시죠?", "서류 복잡해서 제가 직접 정리했습니다" 등 자영업자의 공감 멘트를 녹이세요.
- 🛑 주최자 시점 절대 금지: 본인이 정책을 주최하는 것처럼("저희가 지원해 드려요") 쓰지 마세요. 철저하게 '제3의 정보 전달자(같은 자영업자)' 입장을 유지하세요.
...
5. 🎨 시각화 자료 및 마무리:
- 본문 중간에 \`[🎨 카드뉴스 슬라이드 N번 삽입]\` 기호를 내용의 흐름에 맞춰 분산 배치하세요. (2~7장 등 유동적)
- 맨 마지막에는 \`[📊 최종 요약 인포그래픽 삽입]\` 기호를 1번만 배치하세요.
- 텍스트 모아보기: 원고 작성이 모두 끝난 후, 맨 마지막 줄에 '---' (구분선)을 긋고, 캔바(Canva)에 복사하기 쉽도록 [슬라이드별 핵심 대본]과 [인포그래픽 요약 텍스트]를 한곳에 모아서 출력하세요.
`;
...
export default function Dashboard() {
  ...
  const copySpecificPrompt = useCallback((item: FeedItem) => {
    let promptText = MASTER_PROMPT_V8_1;
