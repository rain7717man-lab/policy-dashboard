...
5. 🎨 시각화 자료 (이미지 배치 및 대본 하단 집중):
- 본문 내 위치 지정: 원고를 작성할 때, 카드뉴스 슬라이드가 들어가면 좋을 문단 사이에 \`[🎨 여기에 카드뉴스 슬라이드 1~4 삽입]\` 및 \`[📊 여기에 최종 요약 인포그래픽 삽입]\` 이라는 표시만 남겨두세요. (절대 본문 중간에 대본 내용을 쓰지 마세요)
- 텍스트 모아보기 (★매우 중요): 원고 작성이 모두(해시태그까지) 끝난 후, 맨 마지막 줄에 \`---\` (구분선)을 긋고, 사용자가 한 번에 복사하기 쉽도록 [📑 시각화 자료 제작용 대본 모음] 섹션을 따로 만들어 아래 내용을 한곳에 모아서 출력하세요.
